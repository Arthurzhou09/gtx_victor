...
def load_split_data(file_path, seed, test: bool):
    ...
    if test is not False: 
        split_data = {
            'test_fluorescence':fluorescence,
            'test_optical_props': optical_props,
            'test_reflectance': reflectance,
            'test_depth': depth,
            'test_concentration_fluor': concentration_fluor,
        }
        splits = ['test']
    else:
...
def load_data(file_path, scale_params, seed, normalize, test = False):
    print("Normalize:", normalize)
    data_by_split = load_split_data(file_path, seed, test)

    result = {}
    mins_maxs_fluorescence = None
    mins_maxs_optical_props = None
    for type in data_by_split.keys():  #['train', 'val', 'test']:
